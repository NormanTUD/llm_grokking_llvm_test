...
class TestLogEpochToRunLoggerIntegration:
    """
    Integration test: verify that _log_epoch_to_run_logger actually
    writes sample files with non-empty predictions.

    This tests the full pipeline: _log_epoch_to_run_logger →
    generate_example_samples → RunLogger.log_samples → disk.
    """

    def test_logged_samples_have_predictions(self, stub_model, stub_tokenizer):
        """
        End-to-end: call _log_epoch_to_run_logger and verify the
        written sample file contains non-empty predicted fields.
        """
        import train as train_module

        tmpdir = tempfile.mkdtemp(prefix="test_sample_log_")
        try:
            # Create a real RunLogger pointing at our temp dir
            from run_logger import RunLogger
            run_logger = RunLogger(base_dir=tmpdir)

            # Set the global current_epoch to a high value (the bug trigger)
            old_epoch = train_module.current_epoch
            train_module.current_epoch = 558
            ...
            try:
                train_module._log_epoch_to_run_logger(
                ...
                train_module.current_epoch = old_epoch

            # Find and read the sample file
            samples_dir = os.path.join(run_logger.path, "samples")
            assert os.path.isdir(samples_dir), (
                f"Samples directory not created: {samples_dir}"
            )

            sample_files = [
                f for f in os.listdir(samples_dir) if f.endswith(".txt")
            ]
            assert len(sample_files) > 0, "No sample files written"

            # Read the sample file and check for non-empty predictions
            ...
            assert total_predictions > 0, (
                f"No 'predicted:' lines found in {sample_file}"
            )
            assert empty_predictions == 0, (
                f"{empty_predictions}/{total_predictions} samples have empty "
                f"predictions in {sample_file} at epoch 558. "
                f"The param_range regression bug is back!\n"
                f"File content:\n{content}"
            )
